...
class PageWWDCViewController: UIViewController {
    ...
    func buildLinks() {
        ...
        var swift = Link(frame: CGRect(x: isIpad ? 100:50, y: view.frame.size.height - (isIpad ? 400:220), width: linkSize, height: linkSize))
        ...
        var wwdc = Link(frame: CGRect(x: view.frame.size.width - (isIpad ? 240:120), y: 170, width: linkSize, height: linkSize))
        wwdc.image = UIImage(named:"me-8")
        wwdc.detailText = "The WWDC 2014 was really awesome, such a wonderful experience!"
        wwdc.addKeyFrame(CGRectOffset(wwdc.frame, isIpad ? 300:200, 0), forTime: time - CGFloat(300))
        wwdc.addKeyFrame(wwdc.frame, forTime: time)
        wwdc.addKeyFrame(topFrame, forTime: time + CGFloat(450))
        wwdc.frame = CGRectOffset(wwdc.frame, isIpad ? 300:200, 0)
        introView?.addElement(wwdc)
        
        var guys = Link(frame: CGRect(x: view.frame.size.width - (isIpad ? 160:80), y: view.frame.size.height - 250, width: linkSize, height: linkSize))
        ...
        var bridge = Link(frame: CGRect(x: isIpad ? 160:80, y: view.frame.size.height - (isIpad ? 220:140), width: linkSize, height: linkSize))
        bridge.image = UIImage(named:"bridge-icon")
        bridge.detailText = "San Francisco is a really nice place to visit 😍. We've made a video of our WWDC14 trip!"
        bridge.linkUrl = "https://www.youtube.com/watch?v=hEcYuOzG3Xc"
        bridge.buttonTitle = "Watch the video!"
