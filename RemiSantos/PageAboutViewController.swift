...
class PageAboutViewController: UIViewController {
    ...
    func buildLinks() {
        ...
        var me = Link(frame: CGRect(x: view.frame.size.width - (isIpad ? 140:70), y: 140, width: linkSize, height: linkSize))
        me.text = "Me"
        me.detailText = "Loving life, be optimistic and never give up, that's how I live. Rollerblade, go out, travel, discover, code/create, enjoy the world."
        me.linkUrl = "http://www.remisantos.com"
        me.addKeyFrame(CGRectOffset(me.frame, 180, 0), forTime: time - CGFloat(300))
        me.addKeyFrame(me.frame, forTime: time)
        me.addKeyFrame(topFrame, forTime: time + CGFloat(450))
        me.frame = CGRectOffset(me.frame, 180, 0)
        introView?.addElement(me)
        
        var github = Link(frame: CGRect(x: isIpad ? 150:100, y: view.frame.size.height - 220, width: linkSize, height: linkSize))
        ...
        github.addKeyFrame(CGRectOffset(github.frame, -250, 0), forTime: time - CGFloat(300))
        github.addKeyFrame(github.frame, forTime: time)
        github.addKeyFrame(topFrame, forTime: time + CGFloat(450))
        github.frame = CGRectOffset(github.frame, -250, 0)
        ...
        var school = Link(frame: CGRect(x: isIpad ? 100:50, y: 160, width: linkSize, height: linkSize))
        school.text = "School"
        school.detailText = "I'm studying at Ingesup - Master degree in computer science - We've been featured on the Oct 2014 keynote as one of the first school to teach Swift!"
        ...
        var fr = Link(frame: CGRect(x: isIpad ? 240:200, y: 170, width: linkSize, height: linkSize))
        ...
        fr.addKeyFrame(CGRectOffset(fr.frame, -350, 0), forTime: time - CGFloat(300))
        fr.addKeyFrame(fr.frame, forTime: time)
        fr.addKeyFrame(topFrame, forTime: time + CGFloat(450))
        fr.frame = CGRectOffset(fr.frame, -350, 0)
