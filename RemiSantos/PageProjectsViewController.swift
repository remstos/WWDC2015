...
class PageProjectsViewController: UIViewController {
    ...
    func buildLinks() {
        let linkSize:CGFloat = isIpad ? 70:50
        ...
        var captain = Link(frame: CGRect(x: isIpad ? 80:40, y: 150, width: linkSize+10, height: linkSize+10))
        captain.image = UIImage(named: "captain-icon")
        captain.detailText = "Bring popular posts from social networks on the same place. Given a hashtag, the #Captain find what's happening on Facebook, Twitter, Instagram and Vine! The best app I've ever done, I spent a lot of time on the design and interactions/transitions. Bonus: the Apple Watch app has just been released!"
        ...
        var thumb = Link(frame: CGRect(x: view.frame.size.width - (isIpad ? 140:70), y: 60, width: linkSize-2, height: linkSize-2))
        ...
        var sofa = Link(frame: CGRect(x: view.frame.size.width - (isIpad ? 200:100), y: 250, width: linkSize, height: linkSize))
        ...
        sofa.addKeyFrame(CGRectOffset(sofa.frame, 300, 0), forTime: time - CGFloat(300))
        sofa.addKeyFrame(sofa.frame, forTime: time)
        sofa.addKeyFrame(topFrame, forTime: time + CGFloat(450))
        sofa.frame = CGRectOffset(sofa.frame, 300, 0)
        introView?.addElement(sofa)
        
        var drawl = Link(frame: CGRect(x: view.frame.size.width - (isIpad ? 300:150), y: view.frame.size.height - 200, width: linkSize+5, height: linkSize+5))
        drawl.image = UIImage(named: "drawl-icon")
        drawl.detailText = "Drawl it like a boss, send it to your Messenger friends. An app for Facebook Messenger Plateform, developed/designed in less than a week (currently waiting for review ;))"
        drawl.linkUrl = "http://getdrawl.com"
        drawl.addKeyFrame(CGRectOffset(drawl.frame, 450, 0), forTime: time - CGFloat(300))
        drawl.addKeyFrame(drawl.frame, forTime: time)
        drawl.addKeyFrame(topFrame, forTime: time + CGFloat(450))
        drawl.frame = CGRectOffset(drawl.frame, 450, 0)
        introView?.addElement(drawl)

        var loopse = Link(frame: CGRect(x: isIpad ? 150:100, y: view.frame.size.height - (isIpad ? 300:150), width: linkSize, height: linkSize))
        loopse.image = UIImage(named: "loopse-icon")
        loopse.appId = "704783915"
        loopse.detailText = "Spots + Friends = Sessions. Find new spots of your favorites extreme sports around you, invite some friends and have a nice session"
        ...
        var peekaboo = Link(frame: CGRect(x: isIpad ? 60:30, y: view.frame.size.height - (isIpad ? 160:80), width: linkSize, height: linkSize))
